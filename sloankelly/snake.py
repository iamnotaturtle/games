...
def drawData(surface, gamedata):
	...
	text = font.render("Lives = %d, Level = %d" %
	                   (gamedata.lives, gamedata.level), 0, (255, 255, 255))
	# named parameters
...
def drawGameOver(surface, gamedata):
	# string, anti-alias?, colour
	text1 = font.render(f"Game Over, score: {gamedata.score}", 0, (255, 255, 255))
	text2 = font.render("Space to play or close the window", 0, (255, 255, 255))
	# named parameters
	textpos1 = text1.get_rect(centerx=surface.get_width()/2,
	                          top=surface.get_height()/2 - 48)
...
def updateGame(gamedata, gameTime):
	...
	if (head.x == gamedata.berry.x and head.y == gamedata.berry.y):
		...
		for i in range(gamedata.segments):
			gamedata.blocks.append(
				Position(gamedata.blocks[lastIdx].x, gamedata.blocks[lastIdx].y))
...
def loadImages():
	wall = pygame.image.load('./resources/projects/Snake/wall.png')
	raspberry = pygame.image.load('./resources/projects/Snake/berry.png')
...
snakemap = loadMapFile('./resources/projects/Snake/map.txt')
data = GameData()
